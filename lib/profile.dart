...
class ProfilePage extends StatefulWidget {
  ...
  // final BaseAuth auth;
  ProfilePage(
      {this.logoutCallback, this.loginCallbackRegister, this.loginCallback});
  @override
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void saveData() async {
    if ((mobileNum.length != 10) || !isNumeric(mobileNum)) {
      ...
      await Firestore.instance.collection('users').document(emailID).updateData(
          {"mobile_number": mobileNum, "state": state}).then((result) {
        print("Data updated");
        res = true;
      }).catchError((onError) {
        print("onError");
